...
public class ProcessGestioneBotteghiniUpdateAction extends DispatchAction {
	public ActionForward get(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String uidBotteghino = request.getParameter ("uid");
		service.deleteLuoghiBotteghini(uidBotteghino);
		service.updateRuolo(uidBotteghino , "2");
		
		for(String uidLuogoInteresse:listaUidLuoghiInteresse) {
			if (request.getParameter ("checkbox" + uidLuogoInteresse) != null) {
				service.insertLuoghiOperatori(uidBotteghino , uidLuogoInteresse);
				service.updateRuolo(uidBotteghino , "4");
			} 
		}
		
		session.setAttribute("uid", null);
		session.setAttribute("listaLuoghiInteresseBotteghino", null);

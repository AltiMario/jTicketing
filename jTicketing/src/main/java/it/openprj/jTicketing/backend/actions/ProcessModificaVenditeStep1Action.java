...
public class ProcessModificaVenditeStep1Action extends DispatchAction {
	...
	public ActionForward get(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ArrayList<ModificaVenditeLuoghiInteresse> listaModificaVenditeLuoghiInteresse;
		User user = (User) session.getAttribute("user");
		BackEndMgr service = ServicesFactory.getInstance().getBackEndMgrMgr();
		listaModificaVenditeLuoghiInteresse = (ArrayList<ModificaVenditeLuoghiInteresse>) service.modificaVenditeLuoghiInteresse(user);
		session.setAttribute("listaModificaVenditeLuoghiInteresse", listaModificaVenditeLuoghiInteresse);

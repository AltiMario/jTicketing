...
import it.openprj.jTicketing.frontend.forms.CartFormBoxOffice;
...
public class ProcessTicketBuyBoxOfficeAction extends Action {

	public ProcessTicketBuyBoxOfficeAction() {
		super();
	}
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// stato premuto il tasto elimina dal carrello
		if (request.getParameter("codiceVerifica") != null) {
			...
			CartFormBoxOffice cartForm = (CartFormBoxOffice) form;
		...
		CartFormBoxOffice cartForm = (CartFormBoxOffice) form;
		//CartFormBoxOffice cartForm1 = (CartFormBoxOffice) form;
		
		//cartForm1 =(CartFormBoxOffice)session.getAttribute("cartFormCategorie");
		//cartForm.setCategoria1(cartForm1.getCategoria1());
		
		
		session.setAttribute("cartFormC", cartForm);
		...
		Turno t = new Turno();
		BackEndMgr serviceTurno = ServicesFactory.getInstance().getBackEndMgrMgr();
		if (idTurno == null){
			idTurno = (String)session.getAttribute("idTurno");
		}
		t = serviceTurno.dettaglioTurno(idTurno);
	    String uidTicket = t.getUidTicket();
		
		if(uidTicket == null){
			uidTicket = (String)session.getAttribute("uidTicket");
		}
		...
		CartFormBoxOffice cartFormBoxOffice = (CartFormBoxOffice) form;
		
		// stato premuto il tasto Scegli altri biglietti
...
//				BackEndMgr serviceTurno = ServicesFactory.getInstance().getBackEndMgrMgr();
//				t = serviceTurno.dettaglioTurno(idTurno);
//			    uidTicket = t.getUidTicket();
				for (int x = 0; x < listaTickets.size(); x++) {
					if(listaTickets.get(x).getUid().equals(t.getUidTicket())){ 
				...
				session.setAttribute("uidTicket", uidTicket);
				request.setAttribute("uidTicket", uidTicket);
		...
		try {
			...
			if (cartForm != null && cartForm.getBtn() != null && cartForm.getBtn().equalsIgnoreCase("Buy")) {
				...
				if (user != null) {
					...
					service.insertTicketAcquistatabiliBotteghino(user, purchasedticketGrouped);
					cartForm.setListaTicketAcquistati(null);

...
<div class="main_center">
	...
	<html:form action="/processGestioneOperatori.do?dispatch=get">
		<tr>
		    <td align="left">
			...
			<td align="left">
				<select name="ruolo">
					...
					%>
						<option value="Operatore" selected="selected"><bean:message key="role.operator"/></option>
						<option value="Utente registrato"><bean:message key="role.userDesc"/></option>
						
					<%
						} else if (ruolo.equals("Utente registrato")) {
					%>
						<option value="Operatore"><bean:message key="role.operator"/></option>
					...
					%>
						<option value="Operatore"><bean:message key="role.operator"/></option>
						<option value="Utente registrato"><bean:message key="role.boxOffice"/></option>
	...
	<logic:present name="listaOperatori" scope="session">
		<logic:iterate id="objElemento" name="listaOperatori" indexId="indiceElemento" scope="session">
				<tr>
				    <td align="left">
...
%>
		<html:form action="/processGestioneOperatoriUpdate.do?dispatch=get">
		<div class="main_center">
			<h3><bean:message key="label_go.selectInterestPlace"/></h3>
			<logic:present name="listaLuoghiInteresseOperatore" scope="session">
				<logic:iterate id="objElemento" name="listaLuoghiInteresseOperatore"
					indexId="indiceElemento" scope="session">
					<div class="left_box2">
					<table width="100%" border="0">
						<tr>
						    <td>
						    	<input name="checkbox<bean:write name="objElemento" property="uid" />" type="checkbox" <bean:write name="objElemento" property="assegnatoOperatore" /> />
						    </td>
						    <td align="left">
			...
			<input type="button" value="<bean:message key="label_go.cancel"/>" class="submit" onclick="javascript:location.href='./processGestioneOperatori.do?dispatch=get'"/>
